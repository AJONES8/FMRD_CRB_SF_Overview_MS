...
vessel_info <- vps_vessels %>% filter(AP_YEAR>2006) %>%
...
Convertimg the decimal degree minutes format in the CRB data to decimal degrees.
...
#Getting the Observer Data from 2007 on
#getting gear codes
...
OBTRP <- dbGetQuery(con.nova, "select * from OBDBS.OBTRP where YEAR > 2006 and YEAR < 2021")
ASMTRP <- dbGetQuery(con.nova, "select * from OBDBS.ASMTRP where YEAR > 2006 and YEAR < 2021")

#ASM and OBS HAUL data for all gears
#OBHAU <- dbGetQuery(con.nova, "select * from OBDBS.OBHAU where YEAR > 2018 and YEAR < 2021")
#ASMHAU <- dbGetQuery(con.nova, "select * from OBDBS.ASMHAU where YEAR > 2018 and YEAR < 2021")

#ASM and OBS haul data for trawl gear
OBHAU <- dbGetQuery(con.nova, "select * from OBDBS.OBHAU where YEAR > 2006 and YEAR < 2021 and negear in('050','051','052','053','054','056','057','058','170','370')")
ASMHAU <- dbGetQuery(con.nova, "select * from OBDBS.ASMHAU where YEAR > 2006 and YEAR < 2021 and negear in('050','051','052','053','054','056','057','058','170','370')")

#ASM and 
OBSPP <- dbGetQuery(con.nova, "select * from OBDBS.OBSPP where YEAR > 2006 and YEAR < 2021")
ASMSPP <- dbGetQuery(con.nova, "select * from OBDBS.ASMSPP where YEAR > 2006 and YEAR < 2021")
...
#Calculating the amount of haddock for each catch record
#Will be zero for hauls without haddock
#changed hail column because no unit conversion
ASM_OB_CATCH_SUB_edit_sum <- ASM_OB_CATCH_SUB_edit_cf %>%
  mutate(haddock_KEPTWT = case_when(SPECIES_ITIS==	'164744' & CATDISP == '1' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         haddock_DISCARDTWT = case_when(SPECIES_ITIS==	'164744' & CATDISP == '0' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         SUM_haddock_CATCH = haddock_KEPTWT + haddock_DISCARDTWT)

#Adding a trip total  
ASM_OB_CATCH_SUB_edit_cf_tot <- ASM_OB_CATCH_SUB_edit_sum %>% group_by(LINK1) %>% 
                                      mutate(TOT_CATCH = sum(HAIL_AMOUNT_LB,na.rm=TRUE),
                                             TOT_haddock_CATCH=sum(SUM_haddock_CATCH,na.rm=TRUE)) %>% ungroup()
...
ASM_OB_CATCH_SUB_edit_sum_sum <- ASM_OB_CATCH_SUB_edit_cf_tot %>% 
                          group_by(LINK1,LINK3,DATESAIL,PERMIT1,YEAR,MONTH,TRIPID,HAULNUM,
                                   AREA,NEMAREA,NEGEAR,OBSRFLAG,LATHBEG,LONHBEG,
                                   HAUCOMMENTS,DATEHBEG,DATEHEND,
                                   GIS_LATHBEG,GIS_LATHEND,GIS_LONHBEG,GIS_LONHEND,HAULDUR,DEPTH,TOT_CATCH,TOT_haddock_CATCH,
                                   TARGSPEC1
                                   #GEAR_SIZE,MESH_SIZE,MESH_TYPE,MESH_SIZE_AB,CODCAT,NUM_HAULS,SOAK_DURATION_DH
                                   ) %>% 
                                   summarise(haddock_KEPTWT = sum(haddock_KEPTWT,na.rm=TRUE),
                                             haddock_DISCARDTWT = sum(haddock_DISCARDTWT,na.rm=TRUE),
                                             SUM_haddock_CATCH = sum(SUM_haddock_CATCH,na.rm=TRUE)
                                             )

#Adding mesh size
ASM_OB_CATCH_SUB_edit_sum_sum_ms <- ASM_OB_CATCH_SUB_edit_sum_sum %>% left_join(.,OBOTGH %>% dplyr::select(LINK3,CODMSIZE,FTROPLEN) %>%  group_by(LINK3,FTROPLEN,CODMSIZE) %>% slice(1) %>% mutate(CODMSIZE=CODMSIZE*0.0393701)) %>% ungroup()
...
#Calculating the amount of Atl haddock for each catch record
#Will be zero for hauls without Atl haddock
#changed hail column because no unit coversion
pulled_data_cf_sum <-pulled_data_edit_cf %>%
  mutate(haddock_KEPTWT = case_when(SPECIES_ITIS==	'164744' & DISPOSITION_CODE == '011' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         haddock_DISCARDTWT = case_when(SPECIES_ITIS==	'164744' & DISPOSITION_CODE == '100' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         SUM_haddock_CATCH = haddock_KEPTWT + haddock_DISCARDTWT)
...
pulled_data_cf_sum_sum <- pulled_data_cf_tot %>% 
                          ...
                          group_by(SOURCE,REPORT_SOURCE,TRIP_ID,EFFORT_ID,VESSEL_PERMIT_NUM,VESSEL_HULL_ID,VESSEL_NAME,
                                   ...
                                   NUM_HAULS,SOAK_DURATION_DH,TOT_CATCH,TOT_haddock_CATCH) %>% 
                                   summarise(haddock_KEPTWT = sum(haddock_KEPTWT,na.rm=TRUE),
                                             haddock_DISCARDTWT = sum(haddock_DISCARDTWT,na.rm=TRUE),
                                             SUM_haddock_CATCH = sum(SUM_haddock_CATCH,na.rm=TRUE)
                                             )
...
pulled_data_cf_sum_sum_sub <- pulled_data_cf_sum_sum_tg %>% ungroup() %>% 
...
ASM_OB_CATCH_SUB_edit_sum_sum_sub <- ASM_OB_CATCH_SUB_edit_sum_sum_ms %>% ungroup() %>% dplyr::select(permit=PERMIT1,area=AREA,
                                                                trip_id=LINK1,
                                                                haul_id=LINK3,
                                                                haul_num=HAULNUM,
                                                                sail_date=DATESAIL,
...
Taking the GPS points (effort/haul beginning and end) and converting them to a line to calculated the length of the tow.
This is unfortunately REALLY not efficient and takes awhile to convert

```{r Converting to SF Format, fig.height=8, fig.width=13}
#Converting the lat lon into distance and SF format
#swept area
#Making an sf formatted haul start
effort.start <- combined_data %>% ungroup() %>% #filter(BSB_CPUE>0) %>%
  mutate(TRIPID_EFFORT=paste(trip_id,haul_num,sep='_')) %>%
  dplyr::select('TRIPID_EFFORT',LON='start_lon',LAT='start_lat') %>%
  filter(LON > -80, LAT > 30, LAT < 45, LON < -60)

#Making an sf formatted haul stop
effort.end <- combined_data %>% ungroup() %>% #filter(BSB_CPUE>0) %>%
  mutate(TRIPID_EFFORT=paste(trip_id,haul_num,sep='_')) %>%
  dplyr::select('TRIPID_EFFORT',LON='end_lon',LAT='end_lat') %>%
  filter(LON > -80, LAT > 30, LAT < 45, LON < -60)

#calculating tow length & filtering out tows without two GPS points
#This could be improved
tow.coords.pre <- bind_rows(effort.start,effort.end) %>% 
  as.tibble() %>%
  drop_na(LAT,LON,TRIPID_EFFORT) %>%
  group_by(TRIPID_EFFORT) %>% 
  distinct() %>% 
  mutate(n=n()) %>% filter(n==2) %>%
  ungroup() %>%
  mutate(LAT=as.numeric(LAT),LON=as.numeric(LON)) %>%
  drop_na(LAT,LON,TRIPID_EFFORT) %>%
  st_as_sf(coords=c('LON','LAT')) %>% #View()
  dplyr::select(-n) %>%
  group_by(TRIPID_EFFORT) %>% 
...
tow.coords <- tow.coords.pre %>% filter(st_geometry_type(.) %in% c('MULTIPOINT'))
tow.coords <- tow.coords %>% st_cast(.,'LINESTRING')
tow.coords <- tow.coords %>% mutate(tow_length=st_length(geometry))

```

##Data polishing 2 (inspecting and weeding out co-sampled trips)
Matching observer and cooperative research trips by permit and date, then comparing the total catch of haddock on a given trip between the two data sources.
...
plot_data <- combined_data %>% 
  mutate(TRIPID_EFFORT=paste(trip_id,haul_num,sep='_')) %>%
  inner_join(.,tow.coords) %>%
  filter(as.numeric(tow_length)<40000,as.numeric(tow_length)>100) %>%
...
##Adding vessel info
#Adding vessel info
plot_data_comb_ves <- plot_data_comb %>% left_join(.,vessel_info %>% mutate(VP_NUM=as.character(VP_NUM)),by=c("permit"="VP_NUM","YEAR"="AP_YEAR"))
